...
class elearning
{
		...
		try {
			if (!empty($userid) && !empty($password)) {
				$stmt = $this->DB->prepare("SELECT USER_ID, `PASSWORD`, '0' AS TYPE FROM tbl_user
                                        ...
                                        UNION SELECT INSTRUCTOR_ID, `PASSWORD`, '1' AS TYPE FROM tbl_instructor
	...
	public function loggedIn($userid, $type)
	{
		if (isset($userid) && isset($type)) {
			if ($type == '0') {
				...
				$stmt = $this->DB->prepare("SELECT * FROM tbl_instructor WHERE INSTRUCTOR_ID = :userid");
				$stmt->bindValue(':userid', $userid);
				$stmt->execute();
	...
	public function setStudentInfo($userid, $first_name, $last_name, $mobile, $dob, $course, $program, $password, $date)
	{
		try {
			if (!empty($userid) && !empty($first_name) && !empty($last_name) && !empty($mobile) && !empty($dob) && !empty($course) && !empty($password) && !empty($password)) {
				$stmt = $this->DB->prepare("INSERT INTO tbl_student(STUDENT_ID, FIRST_NAME, LAST_NAME, MOBILE, DOB, DEPARTMENT,PROGRAM, `PASSWORD`, STATUS, DATE) VALUES(:userid, :firstname, :lastname,:mobile,:dob, :course,:program, :password, 'Registered', :date)");
				$stmt->bindValue(':userid', $userid);
	...
	public function setInstructorInfo($userid, $first_name, $last_name, $password, $date)
	{
		try {
			if (!empty($userid) && !empty($first_name) && !empty($last_name) && !empty($password) && !empty($date)) {
				$stmt = $this->DB->prepare("INSERT INTO tbl_instructor(INSTRUCTOR_ID, FIRST_NAME, LAST_NAME, PASSWORD, STATUS, DATE) VALUES(:userid, :firstname, :lastname, :password, 'Registered', :date)");
				$stmt->bindValue(':userid', $userid);
				$stmt->bindValue(':firstname', $first_name);
				$stmt->bindValue(':lastname', $last_name);
				$stmt->bindValue(':password', $password);
	...
	public function setClass($classcode, $courseid, $instructorid, $syid, $date)
	{
		try {
			if (!empty($classcode) && !empty($courseid) && !empty($instructorid) && !empty($syid) && !empty($date)) {
				$stmt = $this->DB->prepare("INSERT INTO tbl_class(CLASS_CODE, COURSE_ID, INSTRUCTOR_ID, SCHOOL_YEAR_ID, SEMESTER, DATE) VALUES(:classcode, :courseid, :instructorid, :syid, 0, :date)");
	...
	{
		try {
			$stmt = $this->DB->prepare("SELECT * FROM tbl_class AS class INNER JOIN tbl_course AS subj ON class.COURSE_ID = subj.COURSE_ID INNER JOIN tbl_instructor AS fac ON class.INSTRUCTOR_ID = fac.INSTRUCTOR_ID INNER JOIN tbl_school_yr AS sy ON class.SCHOOL_YEAR_ID = sy.ID ORDER BY CLASS_ID DESC");
			$stmt->execute();

			while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
				$id = htmlentities($row['CLASS_ID']);
				$code = htmlentities($row['CLASS_CODE']);
				$subj = htmlentities($row['COURSE_TITLE']);
				$first_name = htmlentities($row['FIRST_NAME']);
				$last_name = htmlentities($row['LAST_NAME']);
	...
	public function getClassesInstructor($userid)
	{
		try {
			$stmt = $this->DB->prepare("SELECT * FROM tbl_class AS class INNER JOIN tbl_course AS subj ON class.COURSE_ID = subj.COURSE_ID INNER JOIN tbl_instructor AS fac ON class.INSTRUCTOR_ID = fac.INSTRUCTOR_ID INNER JOIN tbl_school_yr AS sy ON class.SCHOOL_YEAR_ID = sy.ID WHERE class.INSTRUCTOR_ID = :id ORDER BY CLASS_ID DESC");
			...
			while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
				...
				echo "<tr><td><a href='class.php?cid=" . $id . "' class='view-button'>" . $code . "</a></td><td>" . $subj . "</td><td>" . $first_name . " " . $last_name . "</td><td>" . $sy . "</td><td><a href='classes.php?id=" . $id . "&action=deleteClass' onclick=\"return confirm('Are you sure you want to delete?');\"> Delete</a></td></tr>";
			}
	...
	public function getClassesStudent($userid)
	{
		try {
			...
			while ($row1 = $stmt->fetch(PDO::FETCH_ASSOC)) {
				...
				$stmt2 = $this->DB->prepare("SELECT * FROM tbl_class AS class INNER JOIN tbl_course AS subj ON class.COURSE_ID = subj.COURSE_ID INNER JOIN tbl_instructor AS fac ON class.INSTRUCTOR_ID = fac.INSTRUCTOR_ID INNER JOIN tbl_school_yr AS sy ON class.SCHOOL_YEAR_ID = sy.ID WHERE class.CLASS_ID = :cid ORDER BY CLASS_ID DESC");
	...
	public function getStudentsAd()
	{
		try {
			...
			while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
				...
				$course = htmlentities($row['DEPARTMENT']);
	...
	public function getInstructorAd()
	{
		try {
			$stmt = $this->DB->prepare("SELECT * FROM tbl_instructor");
			$stmt->execute();

			while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
				$fid = htmlentities($row['INSTRUCTOR_ID']);
				$first_name = htmlentities($row['FIRST_NAME']);
				$last_name = htmlentities($row['LAST_NAME']);

				echo "<tr><td>" . $fid . "</td><td>" . $first_name . "</td><td>" . $last_name . "</td><td><a href='instructor_edit.php?id=" . $fid . "&action=select-instructor'>  Edit</a><a href='instructor.php?id=" . $fid . "&action=deleteInstructor' onclick=\"return confirm('Are you sure you want to delete?');\"> Delete</a></td></tr>";
			}
	...
	public function getCourseAd()
	{
		try {
			$stmt = $this->DB->prepare("SELECT * FROM tbl_course");
			$stmt->execute();

			while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
				$id = htmlentities($row['COURSE_ID']);
				$scode = htmlentities($row['COURSE_CODE']);
				$title = htmlentities($row['COURSE_TITLE']);

				echo "<tr><td>" . $scode . "</td><td>" . $title . "</td><td><a href='course_edit.php?id=" . $id . "&action=select-course' class='edit-link'>  Edit</a><a href='course.php?id=" . $scode . "&action=deleteCourse' onclick=\"return confirm('Are you sure you want to delete?');\"  class='delete-link'> Delete</a></td></tr>";
			}
	...
	public function getAnnounceInstructorAd($id, $cid)
	{
		try {
	...
	public function isInstructorId($instructorid)
	{
		$stmt = $this->DB->prepare("SELECT * FROM tbl_instructor WHERE INSTRUCTOR_ID = :instructorid");
		$stmt->bindValue(':instructorid', $instructorid);
		$stmt->execute();

		$row = $stmt->fetch(PDO::FETCH_ASSOC);
		$id = htmlentities($row['INSTRUCTOR_ID']);
	...
	public function isCourseId($courseid)
	{
		$stmt = $this->DB->prepare("SELECT * FROM tbl_course WHERE COURSE_CODE = :subcode");
		$stmt->bindValue(':subcode', $courseid);
		$stmt->execute();

		$row = $stmt->fetch(PDO::FETCH_ASSOC);
		$id = htmlentities($row['COURSE_CODE']);
	...
	public function getInstructorsRegistered()
	{
		try {
			$stmt = $this->DB->prepare("SELECT COUNT(INSTRUCTOR_ID) AS rowCount FROM tbl_instructor WHERE STATUS='Registered'");
			$stmt->execute();
			$row = $stmt->fetch(PDO::FETCH_ASSOC);
			echo $row['rowCount'];
		} catch (Exception $e) {
	...
	public function getInstructorSelect()
	{
		try {
			$stmt = $this->DB->prepare("SELECT * FROM tbl_instructor");
			$stmt->execute();
	...
	public function getInstructorSelectInstructor($id)
	{
		try {
			$stmt = $this->DB->prepare("SELECT * FROM tbl_instructor WHERE INSTRUCTOR_ID = :id");
			$stmt->bindValue(':id', $id);
			$stmt->execute();
	...
	{
		try {
			$stmt = $this->DB->prepare("SELECT * FROM tbl_class WHERE INSTRUCTOR_ID = :id");
			$stmt->bindValue(':id', $id);
			$stmt->execute();
	...
	{
		try {
			$stmt = $this->DB->prepare("DELETE FROM tbl_instructor WHERE INSTRUCTOR_ID = :id");
			$stmt->bindValue(':id', $id);
			$stmt->execute();
		} catch (Exception $e) {
			echo "Something went wrong!";
	...
	{
		try {
			$stmt = $this->DB->prepare("DELETE FROM tbl_course WHERE COURSE_CODE = :id");
			$stmt->bindValue(':id', $id);
			$stmt->execute();
		} catch (Exception $e) {
			echo "Something went wrong!";
	...
	public function getClassInstructor($id)
	{
		try {
			$stmt = $this->DB->prepare("SELECT * FROM tbl_class WHERE CLASS_ID = :id");
			$stmt->bindValue(':id', $id);
			$stmt->execute();
			$row = $stmt->fetch(PDO::FETCH_ASSOC);
			$fid = $row['INSTRUCTOR_ID'];

			$stmt = $this->DB->prepare("SELECT * FROM tbl_instructor WHERE INSTRUCTOR_ID = :fid");
	...
	public function getClassStudentsInstructor($id)
	{
		try {
			$stmt = $this->DB->prepare("SELECT * FROM tbl_class_student AS class_stud INNER JOIN tbl_student AS stud ON class_stud.STUDENT_ID = stud.STUDENT_ID WHERE CLASS_ID = :id ORDER BY CLASS_STUDENT_ID DESC");
			$stmt->bindValue(':id', $id);
	...
	public function getStudents($id, $search = '')
	{
		try {
...
WHERE STUDENT_ID NOT IN (SELECT STUDENT_ID FROM tbl_class_student WHERE CLASS_ID = :id);");
			...
			while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
				...
				#echo "<td>" . $mobile . "</td>";
				echo "<td>" . $course . "</td>";
				echo "<td><a href='students.php?id=" . $id . "&sid=" . $sid . "&action=addToClass'> Add Student</a></td></tr>";
			}
		} catch (Exception $e) {
			echo "Something went wrong!";
		}
	...
	public function getStudentsInstructor($id, $search = '')
	{
		try {
		$query = "SELECT * FROM tbl_student WHERE STUDENT_ID NOT IN (SELECT STUDENT_ID FROM tbl_class_student WHERE CLASS_ID = :id)";
        
        // If the search parameter is set, add the search condition
        if ($search !== '') {
            $query .= " AND (STUDENT_ID LIKE :search OR FIRST_NAME LIKE :search OR LAST_NAME LIKE :search)";
        }
        ...
        // Bind the search value with wildcards for partial matching
        if ($search !== '') {
            $searchTerm = "%$search%";
	...
	public function setCourse($subcode, $title, $date)
	{
		try {
			$stmt = $this->DB->prepare("INSERT INTO tbl_course(COURSE_CODE, COURSE_TITLE,`DATE`) VALUES(:subcode, :title, :date)");
	...
	public function setQuiz($title, $class, $instructor, $date)
	{
		try {
			$stmt = $this->DB->prepare("INSERT INTO tbl_quizes(QUIZ_TITLE, CLASS_ID, INSTRUCTOR_ID, `DATE`) VALUES(:title, :class, :instructor, :date)");
			$stmt->bindValue(':title', $title);
			$stmt->bindValue(':class', $class);
	...
	{
		try {
			$stmt = $this->DB->prepare("UPDATE tbl_student SET STUDENT_ID = :userid, FIRST_NAME = :firstname, LAST_NAME = :lastname, MOBILE = :mobile, DOB = :dob, DEPARTMENT = :course, PASSWORD = :password, `DATE` = :date WHERE STUDENT_ID = :userid");
			$stmt->bindValue(':userid', $userid);
			$stmt->bindValue(':firstname', $first_name);
			$stmt->bindValue(':lastname', $last_name);
			$stmt->bindValue(':mobile', $mobile);
	...
	public function editCourse($subcode, $title, $date)
	{
		try {
			$stmt = $this->DB->prepare("UPDATE tbl_course SET COURSE_CODE = :subcode, COURSE_TITLE = :title,`DATE` = :date WHERE COURSE_CODE = :subcode");
			$stmt->bindValue(':subcode', $subcode);
			$stmt->bindValue(':title', $title);
			$stmt->bindValue(':date', $date);
			$stmt->execute();
	...
	public function editInstructorInfo($userid, $first_name, $last_name, $password, $date)
	{
		try {
			if (!empty($userid) && !empty($first_name) && !empty($last_name) && !empty($password) && !empty($date)) {
				$stmt = $this->DB->prepare("UPDATE tbl_instructor SET INSTRUCTOR_ID = :userid, FIRST_NAME = :firstname, LAST_NAME = :lastname, PASSWORD = :password, `DATE` = :date WHERE INSTRUCTOR_ID = :userid");
				$stmt->bindValue(':userid', $userid);
				$stmt->bindValue(':firstname', $first_name);
				$stmt->bindValue(':lastname', $last_name);
				$stmt->bindValue(':password', $password);
	...
	{
		try {
			$stmt = $this->DB->prepare("SELECT * FROM tbl_course WHERE COURSE_ID = :id");
			$stmt->execute(array(":id" => $id));
			$row = $stmt->fetch(PDO::FETCH_ASSOC);
			$scode = $row['COURSE_CODE'];
